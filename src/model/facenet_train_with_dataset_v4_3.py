...
model = InceptionResNetV2(weights=None, input_tensor=inputs, classes=embedding_size)

model.load_weights('face_model_epoch_19.h5', by_name=True)
...
inputs2 = Input((128,), name='group_input2')

# 图片总数量 = batch_size * 2
x = Lambda(lambda x: tf.reshape(x, [batch_size, 2, embedding_size]), name='prediction_reshape')(inputs2)
# 矩阵相似计算：x^2相似
x = Lambda(similarity, input_shape=[2, embedding_size], name='similarity')(x)
# # 线性回归
x = Dense(1, activation='sigmoid', name='final_predict')(x)
model2 = Model(inputs2, x)

model2.load_weights('face_model_epoch_19.h5', by_name=True)
res = model2.predict(predict)
print(res)
